...
class App(tk.Tk):
    def __init__(self):
        ...
        # create '+', '-', '*', '/' buttons
        for i, text in enumerate(list('+-*/()=c')):
            ...
            grid_row2, grid_column = divmod(i, COLUMN_COUNT)
            button_list[-1].grid(row=grid_row+grid_row2+1, column=grid_column) 

        # error message
        self.message = tk.StringVar() 
        self.label_message = tk.Label(self, text='', textvariable = self.message)
        self.label_message.grid(row=grid_row+grid_row2+2, column=0, columnspan=COLUMN_COUNT)
